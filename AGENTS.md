...
- **`affichage.py`** — `afficher_planning` (color-coded planning, with an "Un jour par ligne" toggle: off = one table per week with nurses as rows, on = a single table with one day per row and one column per nurse) and `stats_depuis_resultat` (summary recomputed from a result dict, used by the CSV viewer). Both the generated planning and the CSV viewer call it, passing a distinct `cle` because two widgets cannot share a key on one page; the choice itself is shared between them via `k_vue_planning` (a bool, persisted as `vue_jours_en_lignes`) and the widget key is re-set from it on every run. The toggle uses an `on_change` callback, not a post-render assignment: `sauvegarde` runs earlier in the script, so without it the preference would be persisted one interaction late.
  `affichage` also owns `COULEURS_DEFAUT` (the per-round palette) and `couleur_tournee`, which reads the user's choice from `k_couleur_T1..T4` in session state. Like `k_wide` and `k_vue_planning`, round colors deliberately do **not** go through `Parametres`: the CSV viewer renders plannings without ever receiving it, and must use the same colors. Text color is no longer hardcoded white — `_texte_sur` picks black or white by BT.601 luminance, because a user-chosen pastel background made white unreadable.
...
- **⚠️ macOS : `PLANNING_NB_WORKERS=1` est obligatoire en local.** Avec les builds Homebrew de Python sur macOS arm64, CP-SAT se bloque définitivement dès `num_workers ≥ 2` : le process reste à 0 % CPU, garé sur une condvar dans `operations_research::sat::NonDeterministicLoop`, et `temps_max` n'y change rien (le budget est appliqué *par* les workers, qui ne démarrent jamais). Ça fige aussi bien `pytest` que le bouton « Générer ». Constaté le 2026-07-20 avec ortools 9.15.6755 sur Homebrew 3.12.13 **et** 3.14.4 ; le Python 3.9 d'Apple (`/usr/bin/python3`) n'est pas touché, Linux/CI non plus (donc la prod va bien). La version de Python n'est pas le facteur — c'est le build. `solveur.py` garde donc `num_workers = 8` par défaut, surchargeable par la variable d'environnement.
- **Ne pas confondre avec le démarrage à froid.** Au premier lancement après une install de dépendances, macOS valide les signatures des binaires fraîchement téléchargés : le process reste figé à 0 % CPU pendant **plusieurs minutes** (mesuré : 123 s pour la suite complète), puis tout redevient instantané (~1 s) pour toujours. Symptôme identique au blocage ci-dessus, cause totalement différente. **`sample <pid>` tranche en 2 secondes** : pile dans `dyld4::…CodeSignatureInFile` → démarrage à froid, laisser tourner ; pile dans `operations_research::sat::…` → blocage, tuer le process. Ne pas conclure « ça ne finira jamais » à partir d'une attente de 20 s : cette confusion a coûté plusieurs diagnostics faux dans les deux sens.
