...
- **`tests/`** — pytest suite (`pytest`, config in `pyproject.toml`), run in CI by `.github/workflows/ci.yml` alongside `ruff check .` and an `import app_planning` smoke test. CI runs a single Python version, 3.14, because that's what Streamlit Community Cloud runs — testing the deployment target beats testing a matrix nobody deploys on. Only Streamlit-free logic is tested; UI functions that call `st.*` are covered indirectly at most. Two things to know before writing solver tests:
